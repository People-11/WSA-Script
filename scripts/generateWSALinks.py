...
for filename, values in identities.items():
    if re.match(fr"MicrosoftCorporationII\.WindowsSubsystemForAndroid_.*\.msixbundle", filename):
        ...
        if (wsa_build_ver == 0):
            ...
            wsa_build_ver = tmp_wsa_build_ver
for filename, values in identities.items():
    if re.match(fr"Microsoft\.UI\.Xaml\..*_{arch}_.*\.appx", filename):
        ...
        out_file_name = f"{values[1]}_{arch}.appx"
        out_file = download_dir / out_file_name
    elif not release_name == 'latest' and re.match(fr"MicrosoftCorporationII\.WindowsSubsystemForAndroid_.*\.msixbundle", filename):
        tmp_wsa_build_ver = re.search(
            r'\d{4}.\d{5}.\d{1,}.\d{1,}', filename).group()
